...
class JwtHelperTest extends TestCase
{
    ...
    public function testdecodeJwt(): void
    {
        ...
        $token = JwtHelper::encodeJwt('HS512',config('settings.admin_jwt.key'), $data, 5);
    ...
    public function testEncodeJwt(): void
    {
        ...
        $data = JwtHelper::decodeJwt('HS512',config('settings.admin_jwt.key'), $token);
        ...
        $data = ['fake' => ' fake',];
        $token = JwtHelper::encodeJwt('HS512',config('settings.admin_jwt.key'), $data, 5);
        $data = JwtHelper::decodeJwt('HS512','', $token);
        self::assertIsArray($data);
        self::assertFalse($data['result_status']);
        self::assertEquals('Invalid signature', $data['result']);
        ...
        $data = JwtHelper::decodeJwt('HS512',config('settings.admin_jwt.key'), 'sokssen');
        self::assertIsArray($data);
        self::assertFalse($data['result_status']);
        self::assertEquals('Wrong number of segments', $data['result']);

        //Token not provided
        $data = JwtHelper::decodeJwt('HS512',config('settings.admin_jwt.key'), '');
        self::assertIsArray($data);
        self::assertFalse($data['result_status']);
        self::assertEquals('Token not provided', $data['result']);

        //Token expired
        $data = ['fake' => ' fake',];
        $token = JwtHelper::encodeJwt('HS512',config('settings.admin_jwt.key'), $data, 0);
        $data = JwtHelper::decodeJwt('HS512',config('settings.admin_jwt.key'), $token);
        self::assertIsArray($data);
        self::assertFalse($data['result_status']);
